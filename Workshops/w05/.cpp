...
namespace seneca {
   ...
   */
   ...
   {
      return isValidAccountNumber(m_number) && m_balance >= 0;
   }

   bool Account::isValidAccountNumber(int accNumber) const
   {
   ...
   Account &Account::operator=(int accNumber)
   {
      ...
      {
         // If the integer value is an invalid account number, then object should be set to an invalid empty state instead.
         setInvalidEmpty();
      }
